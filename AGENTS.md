...
## Testing Guidelines
Automated tests are not present, so plan for manual QA. Load the unpacked extension, watch the service worker console, and download representative files to confirm rule matches, conflict actions, and `${date:...}` substitutions. Clear `chrome.storage.local` (and `chrome.storage.sync` if used) when verifying the first-run migration flow.
